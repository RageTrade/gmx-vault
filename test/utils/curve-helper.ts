...
export const swapEth = async (amount: BigNumber, signer: SignerWithAddress) => {
  const { crv, weth, gauge, usdt, lpToken, triCrypto, crvOracle, lpOracle, uniswapQuoter, curveYieldStrategyTest } =
    await curveYieldStrategyFixture();
  ...
  await weth.connect(wethWhale).transfer(signer.address, ONE.mul(etherToswap));
  await weth.connect(signer).approve(triCrypto.address, ONE.mul(etherToswap));
...
export const swapUsdt = async (amount: BigNumber, signer: SignerWithAddress) => {
  const { crv, weth, gauge, usdt, lpToken, triCrypto, crvOracle, lpOracle, uniswapQuoter, curveYieldStrategyTest } =
    await curveYieldStrategyFixture();
  ...
  const dy = await triCrypto.get_dy(0, 2, POW_SIX.mul(amount));
...
export const accrueFees = async () => {
  const { crv, weth, gauge, usdt, lpToken, triCrypto, crvOracle, lpOracle, uniswapQuoter, curveYieldStrategyTest } = await curveYieldStrategyFixture();
